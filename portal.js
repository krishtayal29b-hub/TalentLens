...
const DEFAULT_JOB = {
  ...
  status: "Active",
  requiredSkills: ["Embeddings", "Vector Search", "Ranking Evaluation", "Python", "LLM Fine-tuning"],
  description:
    "Own the intelligence layer of Redrob's product: the ranking, retrieval, and matching systems that decide what recruiters see when they search for candidates. Production experience with embeddings-based retrieval, vector databases or hybrid search infrastructure, strong Python, and hands-on evaluation-framework design (NDCG, MRR, MAP, A/B testing) required. 5-9 years experience, product-company background preferred over pure consulting or pure research.",
  createdAt: new Date().toISOString(),
};

const SKILL_SYNONYMS = {
  embeddings: ["embedding", "embeddings", "sentence-transformers", "sentence transformers", "bge", "e5 embedding", "dense retrieval"],
  "vector search": ["pinecone", "weaviate", "qdrant", "milvus", "opensearch", "elasticsearch", "faiss", "vector database", "vector db", "vector search", "hybrid search"],
  "ranking evaluation": ["ndcg", "mrr", "map@", "learning to rank", "learning-to-rank", "precision@", "recall@", "a/b test", "eval framework", "evaluation framework", "reranking", "re-ranking"],
  python: ["python"],
  "llm fine-tuning": ["lora", "qlora", "peft", "fine-tuning", "fine tuning", "finetune", "rlhf"],
...
function analyzeResume(resumeText, job, fileName) {
  ...
  const disqualifyingTitleRe = /\b(hr manager|recruiter|talent acquisition|marketing manager|content writer|sales|account manager)\b/i;
  const titleMismatch = disqualifyingTitleRe.test(resumeText) && skillRatio > 0.5;
  const finalScore = titleMismatch ? Math.min(score, 15) : score;
